...
> ⚠️ **המערכת בבנייה מחדש (v2), החל מ-2026-08-31.** המסמך הזה מתאר את הארכיטקטורה הישנה (v1) — חלקים ממנה כבר נמחקו מהקוד. ההוראה המחייבת לקוד הנוכחי היא **[docs/SPEC_V2_ARCHITECTURE.md](docs/SPEC_V2_ARCHITECTURE.md)**; מצב הבנייה מחדש מתועד ב-**[CLAUDE.md](CLAUDE.md)**. תיעוד ה-README הזה יתעדכן בפאזה 10.
...
### מבנה המסך: פס עליון + שלושה טאבים

הממשק בנוי כ"מסך עבודה" (workbench) ולא כדף נחיתה - ניווט בין שלושה טאבים
עם `useState` פשוט (בלי router), כל ה-state המרכזי נשאר ב-`App` ולא נמחק
במעבר בין טאבים.

**פס עליון (sticky):** שם המערכת, שלושת הטאבים, ומימין לשמאל - badge מצב
שוק (כשקיים), badge "מובילה ב-90 הימים" (מ-`GET /api/strategy-league`),
ובורר הבורסה - **משותף לכל הטאבים** (משפיע גם על הסריקה וגם על רשימת
המעקב למחר).

1. **"רשימת מעקב למחר" (ברירת מחדל בכניסה)** - פאנל ה-gap-and-go
   (`GET /api/watchlist/tomorrow`): חותמת "עודכן לאחרונה", chip מקור
   הנתונים (`alpaca+fmp` מול `fmp-universe`), כפתור "רענן עכשיו",
   הדיסקליימר של נתוני סוף יום, וטבלת המועמדים. לכל שורה יש גם עמודת
   "מחיר פתיחה בפועל" לתיעוד ידני של הגאפ בבוקר שאחרי (ראו
   [כלים לבדיקה ידנית](#כלים-לבדיקה-ידנית-מול-כסף-אמיתי)).
2. **"סריקת שוק"** - פריסה דו-עמודתית ממסך 960px ומעלה: עמודת הגדרות
   צרה ו-sticky (רמת סיכון, שיטת השקעה עם tooltips, פילטרים מתקדמים
   מקופלים ב-`<details>`, כפתור סריקה), ועמודת תוצאות רחבה (מקור נתונים,
   מצב שוק, הערת המלצת אסטרטגיה מותנית-regime, טבלת התוצאות). מתחת ל-
   960px - עמודה אחת.
3. **"התיק שלי"** - `PortfolioSection`. לכל אחזקה יש כעת גם עמודת "תשואה מול
   SPY" (`excessReturnPct` - ראו [כלים לבדיקה ידנית](#כלים-לבדיקה-ידנית-מול-כסף-אמיתי)),
   מוצגת בירוק/אדום או "-" לאחזקות ישנות שנוספו לפני התוספת.
...
#### POST /api/watchlist/tomorrow/rerank - דירוג מחדש לפי פרה-מרקט

תוסף ביוזמת משתמש בלבד (כפתור "עדכן לפי פרה-מרקט" בפאנל, docs/SPEC_SHORT_TERM_UPGRADE.md שלב 7) - לא רץ אוטומטית. קורא קריאת snapshot אחת מ-Alpaca (`GET /v2/stocks/snapshots`) לכל הרשימה הקיימת יחד (`watchlistRerankService.js`), משווה את הגאפ בפועל (מחיר עסקה אחרון מול סגירה קודמת) לתחזית מאמש, ומדרג מחדש לפי הגאפ בפועל. כל מועמד מסומן `gapStatus`: `confirmed` (הגאפ החזיק לפחות חצי מהעוצמה שנחזתה, באותו כיוון), `faded` (התהפך או נחלש משמעותית), או `unknown` (אין נתון פרה-מרקט מ-Alpaca לסימול הזה - לא נזרק, רק מסומן כלא ידוע). ללא `ALPACA_API_KEY_ID`/`ALPACA_API_SECRET_KEY`, או ללא נתוני snapshot כלל, מחזיר `{"available": false, "message": "..."}` ב-200 - הפאנל נשאר זהה לקיים. חשוב לזכור: ה-`feed=iex` החינמי מכסה רק חלק ממסחר הפרה-מרקט בפועל.

## כלים לבדיקה ידנית מול כסף אמיתי

שתי תוספות קטנות שמאפשרות למשתמש לבדוק אם ההמלצות שוות משהו, מול תיק
בדיקה קטן באפליקציית מסחר חיצונית - ראו האיפיון המלא ב-
[`docs/SPEC_MANUAL_TESTING_TOOLS.md`](docs/SPEC_MANUAL_TESTING_TOOLS.md).

**תשואה מול SPY באחזקות ("התיק שלי"):** בהוספת אחזקה (`POST
/api/portfolio/holdings`), השרת שומר גם את מחיר ה-SPY הנוכחי
(`spyPriceAtPurchase`), באותו דפוס בדיוק כמו `spyPriceAtScan` ב-
`scanHistoryService.js`. ב-`GET /api/portfolio`, לכל אחזקה שיש לה
`spyPriceAtPurchase` מחושבים `spyReturnPct`/`excessReturnPct` מול מחיר
ה-SPY הנוכחי; אחזקות ישנות בלי הנתון מקבלות `null` (מוצג "-" ב-UI) ולא
קורסות.

**תיעוד מחיר פתיחה בפועל ("רשימת מעקב למחר"):** `POST
/api/watchlist/outcomes` (body: `date`, `ticker`, `actualOpenPrice`,
`modelClosePrice`) שומר/מעדכן (upsert לפי `date`+`ticker`, לא משכפל) כמה
היה מחיר הפתיחה בפועל מול מחיר הסגירה שהמודל השתמש בו לניקוד, ומחשב
`gapAccuracyPct`. `GET /api/watchlist/outcomes?date=YYYY-MM-DD` מחזיר את
כל הרשומות לאותו יום. הנתונים נשמרים ב-
`server/src/data/watchlistOutcomes.json` (לא ב-git, אותו דפוס כמו
`watchlistCache.json`). ב-UI, כל שורה בטבלת רשימת המעקב מקבלת input+כפתור
...
#### REGIME_HISTORY_FILE_PATH

נתיב לקובץ ה-JSON שבו נשמרים snapshots יומיים של מצב השוק לכל בורסה
(`server/src/services/regimeHistoryStore.js`, docs/SPEC_SHORT_TERM_UPGRADE.md
שלב 6) - משמש לחישוב "regime מוחלק" (הרוב מבין עד 3 הימים האחרונים),
כדי שמצב שוק לא יתהפך בין שתי סריקות באותו יום. אותה מדיניות דיסק כמו
שאר ה-stores למעלה.
