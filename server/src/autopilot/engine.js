  ...
  const logAttempt = (row, strategy, reasonCode, at, flags = {}) => {
    ...
    recommendations.recordShadowCandidate({
      ...
      features: {
        ...
        adv20: row.avgDollarVolume20d ?? row.daily?.avgDollarVolume20d ?? null,
        decisionHourNy: Number(new Intl.DateTimeFormat("en-US", { timeZone: "America/New_York", hourCycle: "h23", hour: "2-digit" }).format(new Date(at))),
        priceFreshnessMs: row.price?.at ? Math.max(0, at - Date.parse(row.price.at)) : null,
        missingMask: {
          rvol: row.rvol == null,
          gapPct: row.gapPct == null,
          atrPct: !(row.daily?.atr14 && row.daily?.price),
          adv20: (row.avgDollarVolume20d ?? row.daily?.avgDollarVolume20d) == null,
          decisionHourNy: false,
          priceFreshnessMs: row.price?.at == null,
        },
        sourceVersions: {
          dailyFeed: "sip",
          intradayFeed: "iex",
