...
מערכת סריקת מניות: לקוח React 19/Vite + שרת Node/Express, npm workspaces (`client/`, `server/`). המשתמש בוחר בורסה/סיכון/אסטרטגיה, השרת סורק universe של מניות, מפעיל פילטרים קשיחים ומנקד לפי 7 אסטרטגיות (`server/src/services/strategies.js`), ומחזיר עד 10 תוצאות מדורגות עם הסבר.

חמש אסטרטגיות המשך-מגמה (Micha, Minervini, Ross, Swing Momentum, Small-Cap Breakout) ושתיים **נגד-מגמה** שנוספו 2026-08-12 (`pullback_uptrend`, `mean_reversion_bounce`) — ראו `docs/SPEC_NEW_STRATEGIES.md`. השתיים החדשות **טרם אומתו**.

כל אסטרטגיה מחזירה `scoreBreakdown` — פירוט הגורמים שהרכיבו את הציון (ערך/משקל/תרומה + `detail` עם הנתון שנמדד בפועל), ממוין לפי תרומה. יש טסט שמוודא שסכום התרומות שווה לציון, כדי שה-UI לא יוכל להציג "סיבה" שאינה הסיבה האמיתית.
...
## טבלת הסריקה — עוצבה מחדש (2026-08-12)

הטבלה הרחבה (10-14 עמודות) הוחלפה ב-`ResultCard` (`client/src/App.jsx`): הכרטיס הסגור מציג רק זהות / **למה** (משפט + שני הגורמים החזקים ביותר עם הנתון שנמדד) / ציון אחד / תוכנית מסחר (סטופ, יחס סיכוי-סיכון, כמות). כפתור "למה המניה הזו?" פותח פירוט מלא עם `ScoreBreakdownBars` וכל מה שהיה בעמודות. שום נתון לא הוסר — רק סודר מחדש לפי חשיבות.
