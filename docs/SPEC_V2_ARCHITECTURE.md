...
## 12. החלטות פתוחות למשתמש

1. **מסלול הרצה.** האם v2 עולה בענף נפרד לצד המערכת הקיימת, או מחליף את `main` ישירות?
2. **feed.** האם לשדרג את Alpaca ל-SIP (נפח מלא)? בלי זה, בחירת ה-RVOL — השכבה שנושאת את ה-edge — עובדת על כיסוי חלקי.
3. **ORB.** דורש נתונים תוך-יומיים ותזמון בזמן אמת. לבנות כבר עכשיו מאחורי דגל, או לדחות?
4. **תקופת נייר.** כמה זמן מוכן להריץ בלי כסף אמיתי עד ש-30 עסקאות יצטברו? זו הדרישה היחידה במסמך שתלויה בסבלנות ולא בקוד.
