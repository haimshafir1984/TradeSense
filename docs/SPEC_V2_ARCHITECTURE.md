...
## 14. ממצא חי לאחר יישום — טרם הוכרע (2026-09-01)

**מחיר הכניסה שהמערכת מציגה בזמן שהשוק פתוח הוא מחיר הסגירה של אתמול, לא מחיר חי.**

נובע ישירות מ-§7.1/`alpacaService.getDailyBars`: כשהשוק פתוח, `end` ננעל ליום המסחר הקודם (הגנה מפני נר "בהיווצרות" - תקינה כשלעצמה). המשמעות: כל עוד השוק פתוח (מהפתיחה ועד הסגירה בלילה), `stock.price` וכל הפיצ'רים הנגזרים ממנו (ATR14, ממוצעים, RSI, נפח) **קפואים על ערכי הסגירה של אתמול**, ללא תלות בשעה המדויקת שבה נלחץ "סרוק שוק" בתוך היום. **הדבר היחיד שכן חי בפועל הוא `catalystService`'s `premarketGapPct`** (מבוסס `getSnapshots`, לא `getDailyBars`).

**זה סותר את ההנחה המרומזת ב-§13.2** ("כניסה במחיר הפתיחה לפי התוכנית שהכרטיס הציג") - התוכנית בפועל בנויה על מחיר אתמול, לא על מחיר הפתיחה האמיתי של היום. אומת ישירות מול Alpaca חי (2026-09-01): נר יומי אחרון = סגירת 2026-08-31 (317.14$), בעוד `latestTrade` באותו רגע הראה 316.15$.

**לא תוקן.** אפשרות תיקון: להשתמש במחיר מ-`getSnapshots` (`latestTrade`/`dailyBar`) כמחיר הכניסה בפועל כששוק פתוח, במקום בסגירת הנר היומי האחרון - אותו מנגנון שכבר מזין את `premarketGapPct`. ממתין להחלטת המשתמש.

**עד אז: מחיר הכרטיס הוא הערכה, לא ציטוט לביצוע** - יש לוודא מול מסך הברוקר בפועל לפני הזנת פקודה (עודכן גם ב-`docs/DAILY_RUNBOOK.md`).
